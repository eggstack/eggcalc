...
The codebase must work when assembled by `build_single.py` into a single file. All code must be in one of the seven core modules: `_process.py`, `units.py`, `evaluator.py`, `_protocol.py`, `normalize.py`, `capabilities.py`, `cli.py`. `__main__.py` is a thin entry point, not in the build manifest.
...
- `network.py` (`ip_inspect`, `cidr_inspect`), `encoding.py` (`codec_convert`, `radix_convert`), and `temporal.py` (`datetime_convert`, `cron_inspect`) are standalone leaves: stdlib-only (`ipaddress`, `base64`, `datetime`/`re`), no exact/ deps. Each is exposed as MCP tools (`ip_inspect`, `cidr_inspect`, `codec_convert`, `radix_convert`, `datetime_convert`, `cron_inspect`).

...
- 12 tool profiles: `full`, `default`, `codegg_core_min`, `codegg_core`, `codegg_preflight`, `codegg_patch`, `codegg_config`, `codegg_unicode_security`, `codegg_shell`, `codegg_repo_audit`, `human_math`, `agent_core` (10 front-door tools; recommended general-agent surface, opt-in — `full` stays the default).
...
- **Evals:** `evals/mcp_tool_selection/` holds the 121-case provider-neutral corpus (`cases.json`, `development`/`held_out` splits, all 83 tools referenced) + README + `reports/` baseline. `scripts/measure_mcp_tool_surface.py` measures serialized bytes per exposure (`agent_core/compact` ≈ 11.7 KB vs `full/full` ≈ 118.4 KB, −90.1%); `scripts/score_mcp_tool_selection.py` scores external JSONL rollouts. Tune keywords/scoring on `development` only; never edit held-out expectations to fit. No provider deps in CI.

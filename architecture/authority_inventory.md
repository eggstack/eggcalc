...
|----------|---------------------|-------|-------|
| `MAX_EXPONENT` | `eggcalc/evaluator.py:90` | 10 000 | `test_evaluator` |
| `MAX_FACTORIAL` | `eggcalc/evaluator.py:91` | 1 000 | `test_evaluator` |
| `MAX_NESTING_DEPTH` | `eggcalc/evaluator.py:92` | 100 | `test_evaluator`, `test_unit_dimensions` |
| `MAX_RESULT_VALUE` | `eggcalc/evaluator.py:93` | 1e308 | `test_evaluator` |
| `MAX_RESULT_DIGITS` | `eggcalc/evaluator.py:97` | 4 300 | `test_evaluator` |
| `MAX_SHIFT_COUNT` | `eggcalc/evaluator.py:98` | 50 000 | `test_evaluator` |
| `MAX_INPUT_LENGTH` | `eggcalc/evaluator.py:99` | 10 000 | `test_evaluator` |
| `MAX_USER_VARIABLES` | `eggcalc/evaluator.py:100` | 1 000 | `test_evaluator` |
| `MAX_NAMED_REGISTERS` | `eggcalc/evaluator.py:101` | 1 000 | `test_evaluator` |
| `DEFAULT_CACHE_SIZE` | `eggcalc/evaluator.py:102` | 1 024 | `test_evaluator` |
| `MAX_CACHE_BYTES` | `eggcalc/evaluator.py:103` | 64 MB | `test_evaluator` |
| `MAX_ORPHANED_PROCESSES` | `eggcalc/evaluator.py:587` | 256 | `test_evaluator` |
...
| `MAX_NESTING_DEPTH` | Re-exported from `eggcalc.evaluator:92` | 100 | `test_normalize` |
...
|----------|---------------------|---------|-------|
| `MAX_REQUEST_BYTES` | `eggcalc/mcp/server.py:128` | `EGGCALC_MCP_MAX_REQUEST_BYTES` | `test_mcp_server` |
| `MAX_OUTPUT_BYTES` | `eggcalc/mcp/server.py:129` | `EGGCALC_MCP_MAX_OUTPUT_BYTES` | `test_mcp_server` |
| `MAX_REQUESTS_PER_SECOND` | `eggcalc/mcp/server.py:130` | `EGGCALC_MCP_MAX_REQUESTS_PER_SECOND` | `test_mcp_server` |
| `MAX_REQUEST_ID_LENGTH` | `eggcalc/mcp/server.py:131` | — | `test_mcp_server` |
| `MAX_TOOL_TIMEOUT_SECONDS` | `eggcalc/mcp/server.py:132` | `EGGCALC_MCP_MAX_TOOL_TIMEOUT_SECONDS` | `test_mcp_server` |
| `MAX_CANCELLED_REQUESTS` | `eggcalc/mcp/server.py:133` | `EGGCALC_MCP_MAX_CANCELLED_REQUESTS` | `test_mcp_server` |
| `_MAX_TOOL_WORKERS` | `eggcalc/mcp/server.py:264` | `EGGCALC_MCP_MAX_TOOL_WORKERS` | `test_mcp_server` |
| `SUPPORTED_SCHEMA_KEYWORDS` | `eggcalc/mcp/server.py:158` | — | `test_mcp_schema_lint` |
...
| Item | Authoritative source | Tests |
|------|---------------------|-------|
| `UNIT_DEFINITIONS` (declarative source, 150 `UnitSpec`) | `eggcalc/units.py:276` | `test_unit_namespace`, `test_unit_dimensions` |
| `UNIT_BASE` (base-unit factor tables) | Generated adapter from `UNIT_DEFINITIONS`, installed by `_install_generated_adapters()` (`eggcalc/units.py:3587`; import-time install at `:3886`) | `test_unit_namespace`, `test_unit_dimensions` |
| `UNIT_ALIASES` (user-facing → canonical) | Generated adapter (placeholders at `eggcalc/units.py:3354-3357`, populated at import) | `test_unit_namespace`, `test_unit_dimensions` |
| `UNIT_CATEGORIES` (unit → category) | Generated adapter (placeholders at `eggcalc/units.py:3354-3357`, populated at import) | `test_unit_dimensions` |
| `TEMPERATURE_CONVERSIONS` | Generated adapter (placeholders at `eggcalc/units.py:3354-3357`, populated at import) | `test_unit_dimensions` |
| `UNIT_CONVERSIONS` (pairwise factors) | Lazy `_LazyUnitConversions` (`eggcalc/units.py:3366`), computed on demand from the registry | `test_unit_dimensions` |
| `UnitRegistry` (structural registry) | `eggcalc/units.py:2813` (built by `build_unit_registry()` at `:2910`; lazy module instance via `_get_unit_registry()`) | `test_unit_dimensions` |

## Tool Definitions (MCP) — Plan 040 consolidated authorities
| Item | Authoritative source | Adapters / derived views | Tests |
|------|---------------------|--------------------------|-------|
| Catalog: names + handler locators + selection metadata (category/tier/tags/profiles/aliases/exposure/harness/cost/stability/composite/selection_summary/keywords) | `eggcalc/mcp/schemas.py::TOOL_METADATA` (validated by `_validate_catalog_metadata()`) | `TOOL_HANDLERS` derived via `server._build_tool_handlers()`; `get_tool_tier()`/`get_tool_tags()`/`get_tool_handler_name()`/`get_tool_selection_summary()`/`get_tool_keywords()` accessors | `test_tool_inventory` (catalog/protocol key parity, handler identity, tier/tag single authority), `test_mcp_tool_discovery::TestSelectionMetadata` |
| Protocol shape: description/inputSchema/outputSchema/deprecated | `eggcalc/mcp/schemas.py::TOOL_SCHEMAS` (no tier/tags copies) | `compact_schema()`/`normal_schema()` transforms; `tools/list` full/normal/compact rendering (compact description = authored `selection_summary`) | `test_mcp_schema_lint`, `test_tool_inventory::TestTierConsistency`, `test_tool_inventory::TestStandardToolShape`, `test_mcp_tool_discovery::TestCompactContract` |
...
| Profile lists | Derived `eggcalc/mcp/schemas.py::TOOL_PROFILES` via `_build_profiles()` from catalog (12 profiles incl. `agent_core`) | `ToolRegistry.profiles` (frozen); `get_profile_tools()` | `test_tool_inventory`, `test_mcp_server`, `test_mcp_tool_discovery::TestAgentCoreProfile` |
| Lexical discovery ranking | `eggcalc/mcp/server.py::ToolRegistry.search_tools()` + `ToolMatch` (integer weights, canonical-name tie-break; harness/library facility, not protocol) | `scripts/measure_mcp_tool_surface.py --discover`; `evals/mcp_tool_selection/` corpus and evaluation-only candidate sets; `scripts/score_mcp_tool_selection.py` and `scripts/analyze_mcp_tool_selection_failures.py` | `test_mcp_tool_discovery::TestCatalogSearch`, `test_build_single` (package/single-file parity) |
...
| Item | Authoritative source | Tests |
|------|---------------------|-------|
| `COMMANDS` (text command registry) | `eggcalc/cli.py:70` | `test_import_boundaries::TestCommandRegistry` |
| `_COMMAND_NAME_TO_SPEC` (lookup) | `eggcalc/cli.py:181` | `test_import_boundaries::TestCommandRegistry` |
...
| `EvaluationError` | `eggcalc/evaluator.py:1818` | `test_evaluator` |
| `TimeoutError` (eval spawn) | `eggcalc/evaluator.py:3272` | `test_evaluator` |

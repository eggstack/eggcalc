    ...
    for spec in defs:
        ...
        for alias in spec.aliases:
            ...
            if alias in all_aliases:
                existing = all_aliases[alias]
                if existing != spec.canonical:
                    raise ValueError(
                        f"Alias collision: {alias!r} maps to both "
                        f"{existing!r} and {spec.canonical!r}"
                    )
            all_aliases[alias] = spec.canonical
        if spec.scale_to_base == 0:
            raise ValueError(f"Zero scale for {spec.canonical}")
...
# ---------------------------------------------------------------------------
# Structural UnitExpression (Workstream F)
# ---------------------------------------------------------------------------
...
class UnitExpression:
    """Immutable structural representation of a compound unit expression."""
...
def _resolve_atom(unit_str: str) -> tuple[str, Dimension, float] | None:
    """Resolve a single unit atom to (canonical, dimension, scale_to_base).

    Returns None if the unit is unrecognized.
    """
    normalized = normalize_unit(unit_str)
    # Try UNIT_ALIASES first
    canonical = UNIT_ALIASES.get(normalized, normalized)
    _reg = _get_unit_registry()
    if _reg is not None:
        ud = _reg.by_alias(normalized) or _reg.by_alias(canonical)
        if ud is not None:
            return (ud.canonical, ud.dimension, ud.scale)
    # Try UNIT_BASE lookup
    for base_key, variants in UNIT_BASE.items():
        if normalized in variants:
            dim = _CATEGORY_DIMENSIONS.get(base_key, DIM_DIMENSIONLESS)
            return (normalized, dim, variants[normalized])
    return None


def parse_unit_expression(unit_str: str) -> UnitExpression:
    """Parse a compound unit string into a frozen UnitExpression.

    Handles forms like ``"m/s"``, ``"kg*m/s**2"``, ``"m**2"``.
    Rejects ``"//"`` and ``"%"`` as separators.
    Enforces resource bounds: MAX_UNIT_STRING_LENGTH, MAX_COMPOUND_DEPTH,
    MAX_COMPOUND_ATOMS, MAX_ABS_UNIT_EXPONENT.
    Fully consumes input — raises ValueError on leftover characters.
    """
    if not unit_str:
        raise ValueError("Empty unit string")
    if len(unit_str) > MAX_UNIT_STRING_LENGTH:
        raise ValueError(f"Unit string exceeds {MAX_UNIT_STRING_LENGTH} characters")
    if "//" in unit_str or "%" in unit_str:
        raise ValueError(
            f"Unit separators '//' and '%' are not allowed in unit expressions: {unit_str!r}"
        )

    sig = _parse_compound_signature(unit_str)
    if sig is None:
        raise ValueError(f"Cannot parse unit expression: {unit_str!r}")
    ...
    for base, exp in num:
        resolved = _resolve_atom(base)
        if resolved is None:
            raise ValueError(f"Unknown unit: {base!r}")
        can, dim, scale = resolved
        if abs(exp) > MAX_ABS_UNIT_EXPONENT:
            raise ValueError(
                f"Exponent {exp} for {base!r} exceeds MAX_ABS_UNIT_EXPONENT={MAX_ABS_UNIT_EXPONENT}"
            )
        factors[can] = factors.get(can, 0) + exp
        dimension = dimension * (dim**exp)
        scale_to_base *= scale**exp
    ...
    # Drop zero exponents and sort
    sorted_factors = tuple(
        sorted(((b, e) for b, e in factors.items() if e != 0), key=lambda x: x[0])
    )

    return UnitExpression(factors=sorted_factors, dimension=dimension, scale_to_base=scale_to_base)

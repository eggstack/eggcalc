...
class TestUnitSpacingProbes:
    ...
    @pytest.mark.parametrize(
        ("expr", "expected_normalized"),
        [
            ("7m/s//1s", "(7*m/s)//(1*s)"),
            ("7m/s % 2s", "(7*m/s)%(2*s)"),
        ],
    )
    def test_compound_unit_floor_mod_spacing(self, expr, expected_normalized):
        normalized, code = normalize_expression(expr, NORMALIZE, PATTERNS)
        assert code == 0
        assert normalized == expected_normalized
        # Incompatible-dimension floor/mod must be rejected (no compound unit strings).
        _result, code, _out, _err = _run(expr)
        assert code != 0
    ...
    def test_chained_compound_unit_floor_mod_spacing(self):
        ...
        # Incompatible-dimension floor/mod must be rejected.
        _result, code, _out, _err = _run("7m/s//1s//1s")
        assert code != 0

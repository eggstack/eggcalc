...
This directory contains bounded plans handed directly to implementation agents. Adapted from `codegg/plans/implementation/README.md`.
...
Subsystems: `calculator-core`, `exact-utilities`, `mcp-server`, `cli-distribution`. Milestone numbering is local to the subsystem roadmap unless the roadmap defines another stable identifier.
...
Describe the relevant code, tests, packaging, protocol, guards, and known gaps at the repository baseline.
...
State expected behavior for partial failure, duplicate delivery, process restart, cancellation races, stale generations, queue overflow, oversized I/O, and concurrent callers.
...
```bash
# narrow tests first
.venv/bin/python -m pytest tests/test_<area>.py -v
...
- the evaluation path, protocol era, profile contract, or build boundary would need redesign.
...
Include known hazards, resource constraints, serial-test requirements, environment requirements (always use `.venv/bin/python`; system python lacks pytest), and preserved user changes.
```
...
The implementation agent may adjust file-level mechanics based on current code. It may not weaken canonical invariants or silently enlarge scope. It MUST use `evaluate` vs `evaluate_raw`/`run`/CLI correctly, keep runtime code inside core/`exact/`/`mcp/`, never reference `normalize_main` in source/tests, and never hand-edit `docs/tool_inventory.md` or `confusables.py` data.

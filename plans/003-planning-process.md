# eggcalc Planning and Agent-Handoff Process

Status: normative planning governance

This document defines how eggcalc's long-term architecture is translated into actionable work without allowing short-lived implementation details to destabilize the canonical specification, terminology, or master roadmap. Adapted from `codegg/plans/003-planning-process.md` for a stdlib-only Python calculator/MCP project.

The keywords MUST, MUST NOT, REQUIRED, SHOULD, SHOULD NOT, and MAY are normative.

## 1. Purpose

eggcalc requires two distinct planning horizons:
...
An ADR is normally required when a decision changes an evaluation path, unit-dimension rule, catalog authority, protocol-era model, profile contract, import/build boundary, or config-trust boundary. An ADR is usually unnecessary for local refactors, internal naming cleanup, or reversible optimizations that preserve established contracts.

### 2.3 Subsystem roadmaps

A subsystem roadmap translates relevant long-term requirements into one coherent workstream. It is longer-lived than an implementation plan but more adaptable than the canonical roadmap. Stable subsystems: `calculator-core`, `exact-utilities`, `mcp-server`, `cli-distribution`.
...
- cross-cutting security, migration, protocol, packaging, and observability concerns;
...
- packaging, protocol, migration, and compatibility effects;
...
Completed, superseded, or abandoned interim plans SHOULD move under `plans/archive/` once they are no longer active. Archive moves MUST preserve traceability and SHOULD retain original filenames and subsystem grouping. `plans/archive/legacy/` holds the pre-adoption flat plans moved verbatim.
...
- stdlib-only production runtime;
- side-effect-free `import eggcalc`;
- two-evaluation-path contract and caret semantics;
- structural angle-algebra rejection;
- `TOOL_METADATA` catalog authority and `protocol_era()` version authority;
- result-only CLI output;
- lazy config loading.
...
- NL/unit evaluation from CLI, library, and single file;
- deterministic text-analysis functions;
- dual-era MCP discovery and tool calls with typed results;
- curated profiles and catalog search;
- `--explain` traces and `--commands` discovery.
...
- `MODULE_MANIFEST` assembly and manifest validation;
- `ToolRegistry`/`ToolExecutor` bounded pools and reservation state machine;
- `ConfigManager` atomic snapshots;
- normalization-table rebuild locks and evaluator caches.
...
- **operational** — implementation can land, but packaging, docs, or release depends on external evidence.
...
A milestone is too large when it combines several independently releasable capability boundaries, requires unrelated schema/profile migrations, or contains several unresolved architecture decisions.
...
- use the correct evaluation path (`evaluate` vs `evaluate_raw`/`run`/CLI) for the task;
- keep runtime code inside core, `exact/`, or `mcp/` and imports in top-level multi-line parenthesized blocks;
- never reference `normalize_main` in source/tests;
- never hand-edit `docs/tool_inventory.md` or `exact/confusables.py` data;
...
7. timeout, cancellation, restart, contention, and failure semantics;
...
eggcalc review MUST also confirm: evaluation-path selection, stdlib-only impact, lazy-import impact, single-file manifest impact, generated-docs/fixture regeneration needs, and CLI output-shape impact.
...
- for eggcalc specifically: testing NL/units through `evaluate()`; referencing `normalize_main` in source; hand-editing `docs/tool_inventory.md`, `TOOL_HANDLERS`/`TOOL_PROFILES`, or `confusables.py` data; loading cwd config at import time.

## 12. eggcalc verification mapping

Canonical verification for closure evidence (adapt commands to the milestone scope; do not claim unrun commands):
...
`make check` order: lint -> format-check -> typecheck (+ strict consumer) -> docs-check -> build_single --validate -> pytest. CI runs `make check` then `make package-check`.

# eggcalc Planning System

This directory separates durable architectural direction from temporary execution planning.
Adapted from the `codegg` planning conventions (`codegg/plans/README.md`,
`codegg/plans/003-planning-process.md`).
...
- `archive/` — completed or superseded interim planning retained for traceability. `archive/legacy/` holds the pre-adoption flat `plans/*.md` files (001–043 plus dated/phase files), moved verbatim with `git mv` to preserve history.
- `registry.md` — compact index of active subsystem roadmaps, implementation plans, closure work, and dependencies.

## Core rule

Long-term documents state **what eggcalc is becoming and what must remain true**. Interim documents state **what an agent should implement next against a specific repository baseline**.
...
Use stable subsystem names: `calculator-core`, `exact-utilities`, `mcp-server`, `cli-distribution`. Do not encode dates in filenames unless the document is inherently time-bound.
